...
export default function HomePage() {
	...
	return (
		<div className="p-12 max-w-7xl mx-auto flex flex-col gap-3 relative">
			<div className="flex justify-end items-center gap-3 sticky top-2 z-50">
				<Dropdown label={platform}>
					{platforms.map((platform, i) => {
						return (
							<Dropdown.Item key={platform + i} onClick={() => setPlatform(platform)}>
								{platform}
							</Dropdown.Item>
						);
					})}
			...
			{platform === "CodeForces" ? <CodeForcesUsersTable batch={batch} /> : platform === "CodeChef" ? <CodeChefUsersTable batch={batch} /> : <></>}
		</div>
	);
}
